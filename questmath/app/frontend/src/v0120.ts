...
let loading=false;
async function enhance(){if(loading||!token())return;const page=document.querySelector('.page');const isParent=Boolean(document.querySelector('.parent-title'));if(!page||!isParent||document.querySelector('.mq-v0120-history'))return;loading=true;try{const history=await api('/worksheets/history');const todayKey=new Date().toLocaleDateString('en-CA');const today=history.filter((x:any)=>x.date===todayKey);const completed=today.filter((x:any)=>x.completed_at);const answered=today.reduce((n:number,x:any)=>n+(x.answered||0),0);const correct=today.reduce((n:number,x:any)=>n+(x.score||0),0);const hints=today.reduce((n:number,x:any)=>n+(x.hints||0),0);const xp=today.reduce((n:number,x:any)=>n+(x.xp_earned||0),0);const section=document.createElement('section');section.className='panel mq-v0120-history';section.innerHTML=`<div class="mq-v0120-head"><div><p class="eyebrow">TODAY’S LEARNING</p><h2>Worksheet activity</h2><p>${completed.length?`${completed.length} worksheet${completed.length===1?'':'s'} completed today.`:'No worksheet has been completed today yet.'}</p></div></div><div class="mq-v0120-metrics"><article><small>Completed</small><strong>${completed.length}</strong></article><article><small>Questions</small><strong>${answered}</strong></article><article><small>Accuracy</small><strong>${answered?Math.round(correct/answered*100)+'%':'—'}</strong></article><article><small>Hints</small><strong>${hints}</strong></article><article><small>XP today</small><strong>${xp}</strong></article></div>${history.length?`<div class="mq-v0120-list">${historyRows(history,true)}</div>`:'<p>No worksheets recorded yet.</p>'}`;page.querySelector('.parent-title')?.insertAdjacentElement('afterend',section);section.querySelectorAll('[data-review]').forEach((b:any)=>b.addEventListener('click',()=>review(Number(b.dataset.review))))}catch(e){console.warn('MathQuest parent history unavailable',e)}finally{loading=false}}

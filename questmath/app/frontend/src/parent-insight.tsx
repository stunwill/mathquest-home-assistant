...
const collection = (candidate: unknown): any[] => Array.isArray(candidate) ? candidate : [];

export function ParentLearningInsight({data}: {data: any}) {
  if (!data) return null;
  const week = data.weekly?.current || {};
  const level = data.estimated_level || {};
  const gains = collection(data.gains);
  const persistentGaps = collection(data.persistent_gaps);
  const strategiesUsed = collection(data.strategies_used);
  const outcomes = collection(data.outcomes);
  return <>

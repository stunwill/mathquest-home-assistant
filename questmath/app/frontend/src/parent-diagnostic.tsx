...
export function ParentDiagnosticInsight(){
  ...
  return <section className="panel diagnostic-placement" aria-labelledby="parent-diagnostic-title">
    ...
    <div className="diagnostic-placement-grid">
      <article><div><small>STRONGER DIAGNOSTIC EVIDENCE</small>{(data.demonstrated||[]).length?(data.demonstrated||[]).map((item:any)=><p key={item.code}><b>{item.code} · {item.title}</b><span>{friendly(item.target_skill)} · {item.historical_questions} total historical questions</span></p>):<p>No sampled area has enough independent diagnostic evidence to describe as stronger yet.</p>}</div></article>
      <article><div><small>SUPPORTED DURING DIAGNOSTIC</small>{(data.supported||[]).length?(data.supported||[]).map((item:any)=><p key={item.code}><b>{item.code} · {item.title}</b><span>{item.eventual_correct}/{item.diagnostic_questions} eventual correct · support used {item.support_used} time(s)</span></p>):<p>No sampled area required recorded support.</p>}</div></article>
      <article><div><small>MORE EVIDENCE NEEDED</small>{(data.needs_more_evidence||[]).length?(data.needs_more_evidence||[]).map((item:any)=><p key={item.code}><b>{item.code} · {item.title}</b><span>{item.independent_correct}/{item.diagnostic_questions} independent correct · {item.historical_questions} historical questions</span></p>):<p>No sampled area is currently flagged here.</p>}</div></article>
    </div>

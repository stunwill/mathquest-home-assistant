...
export function WorksheetHistory({onCreate, onOpen}:{onCreate: () => void; onOpen: (worksheet: any) => void}) {
...
export function LearningCalendar({onOpen}:{onOpen: (worksheet: any) => void}) {
  ...
  return <section className="panel completion-calendar mq-v0160-calendar"><div className="mq-cal-head"><button type="button" onClick={() => shift(-7)}>« 1 week</button><button type="button" onClick={() => shift(-1)}>‹ 1 day</button><h2>{data ? `${dateLabel(data.start)} – ${dateLabel(data.end)}` : 'Completion calendar'}</h2><button type="button" disabled={rangeStart >= currentMonday} onClick={() => shift(1)}>1 day ›</button><button type="button" disabled={rangeStart >= currentMonday} onClick={() => shift(7)}>1 week »</button></div>
...
export function StoryAdventures({onOpen}:{onOpen: (worksheet: any) => void}) {
  ...
  return <section className="panel mq-v090-adventures"><p className="eyebrow">STORY ADVENTURES</p><h2><BookOpen size={22}/> Practise maths inside a story</h2><p>Each adventure creates a dedicated themed worksheet while MathQuest still tracks the underlying maths skills.</p>{error && <ErrorNotice message={error} retry={load} dismiss={() => setError('')}/>}<div className="mq-adventure-grid">{items === null && !error ? <p>Loading adventures…</p> : items?.map(item => <button type="button" data-theme={item.id} key={item.id} disabled={!!busy} onClick={() => start(item.id)}><span>{item.icon}</span><b>{item.title}</b><small>{busy === item.id ? 'Building your story…' : item.intro}</small>{busy === item.id && <Play size={16}/>}</button>)}</div></section>;
}

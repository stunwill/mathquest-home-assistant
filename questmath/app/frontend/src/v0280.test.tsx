...
describe('Math Mentor', () => {
  it('is available as a collapsible, keyboard-accessible panel on every worksheet question', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      ...
      if (url.includes('/math-mentor?action=why')) return response(mentor);
    ...
    expect(fetchMock.mock.calls.some(call => String(call[0]).includes('/math-mentor?action=why'))).toBe(true);
  });

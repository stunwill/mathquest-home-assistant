...
import {
  CheckCircle2, ChevronRight, Database, Download, Eye, Flame, List,
  LogOut, Play, Settings, SkipForward, Star, Trophy, X
} from 'lucide-react';
import './styles.css';
...
function App(){
  ...
  if(loading)return <div className="splash">MathQuest</div>;
...
function Login({onLogin}:{onLogin:(u:User)=>void}){
  ...
  return <main className="login"><section className="login-card">
    <div className="logo">Q</div><h1>MathQuest</h1><p>Sienna’s daily adventure in maths.</p>
    <form onSubmit={login}>
...
const Header=({user,logout}:{user:User;logout:()=>void})=><header>
  <b>MathQuest <small>v{VERSION}</small></b><span>{user.display_name}</span>
...
function Worksheet({ws,onUpdate,onExit,onDone}:{ws:WorksheetData;onUpdate:(x:WorksheetData)=>void;onExit:()=>void;onDone:(x:any)=>void}){
  ...
  async function refresh(updated?:WorksheetData){const data=updated||await req('/worksheets/today');onUpdate(data);setAnswer('');setFeedback(null);setQuestionStart(Date.now())}
  ...
  return <main className="worksheet-shell">
    <div className="worksheet-header"><button className="ghost danger-text" onClick={()=>setConfirmExit(true)}><X size={20}/> Exit worksheet</button><div className="quest-path"><span>🏠</span><i/><span>🌲</span><i/><span>🏔️</span><i/><span>🏰</span></div><button className="ghost" onClick={()=>setOverview(true)}><List size={20}/> Questions</button></div>
...
function WorksheetStatus({ws,q,open}:{ws:WorksheetData;q:Question;open:()=>void}){return <aside className="worksheet-status"><p className="eyebrow">TODAY’S QUEST</p><h2>{ws.current_phase==='skipped'?'Skipped round':`Question ${q.position+1} / ${ws.total}`}</h2><div className="progress"><i style={{width:`${(ws.counts.correct+ws.counts.incorrect)/ws.total*100}%`}}/></div><dl><div><dt>Completed</dt><dd>{ws.counts.correct+ws.counts.incorrect}</dd></div><div><dt>Correct</dt><dd className="green">{ws.counts.correct}</dd></div><div><dt>Incorrect</dt><dd className="red">{ws.counts.incorrect}</dd></div><div><dt>Skipped</dt><dd className="amber">{ws.counts.skipped}</dd></div><div><dt>Remaining</dt><dd>{ws.counts.remaining}</dd></div></dl><button className="wide" onClick={open}><Eye size={18}/> View all questions</button></aside>}
...
function Parent({user,logout}:{user:User;logout:()=>void}){
  ...
  useEffect(load,[]);if(!d||!settings)return <div className="splash">Loading parent dashboard…</div>;
  ...
  return <><Header user={user} logout={logout}/><main className="page"><section className="parent-title"><div><p className="eyebrow">PARENT VIEW · VICTORIAN CURRICULUM LEVEL 4</p><h1>Sienna’s learning overview</h1><p>Evidence is grouped by VCAA content description. Status is based on recent first-attempt accuracy and should be considered alongside school assessment.</p></div><div className="actions"><button onClick={()=>download('/reports/progress.csv','mathquest-progress.csv')}><Download size={17}/> CSV</button><button onClick={()=>download('/reports/progress.pdf','mathquest-progress.pdf')}><Download size={17}/> PDF</button></div></section>
  <section className="cards"><Metric label="Streak" value={`${d.streak} days`}/><Metric label="Accuracy" value={`${d.accuracy}%`}/><Metric label="Answered" value={d.questions_answered}/><Metric label="Needs support" value={d.curriculum_summary.needs_support}/></section>

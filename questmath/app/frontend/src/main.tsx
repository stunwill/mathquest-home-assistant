...
export function Worksheet({ws,onUpdate,onExit,onDone}:{ws:WorksheetData;onUpdate:(x:WorksheetData)=>void;onExit:()=>void;onDone:(x:any)=>void}){
  ...
  useEffect(()=>{
    const keydown=(event:KeyboardEvent)=>{
      ...
      if((!feedback||feedback.retry_allowed)&&target instanceof HTMLInputElement&&String(answer??'').trim()){event.preventDefault();void safe(submit);}
    };
  ...
  return <main className="worksheet-shell">
    ...
    <div className="worksheet-layout"><section className="worksheet-main">
      ...
      <section className="question-card" key={`${q.id}:${q.payload?.visual_key||''}`} data-question-id={q.id} data-visual-key={q.payload?.visual_key} data-guided-tutor-owner="true">{actionError&&<ErrorNotice message={actionError} dismiss={()=>setActionError('')}/>} {q.payload?.adventure&&<StoryMissionProgress adventure={q.payload.adventure}/>}<InterventionGoal question={q}/><div className="question-icon">{({number:'🔢',algebra:'□',measurement:'📏',space:'⬡',statistics:'📊',probability:'🎲'} as any)[q.topic]||'✦'}</div><QuestionTools question={q} onOpenLab={()=>setLabOpen(true)}/><h1>{q.prompt}</h1><QuestionVisual question={q}/>{q.payload?.shape&&<FractionShape parts={q.payload.shape.parts} shaded={q.payload.shape.shaded}/>}<Answer q={q} value={answer} setValue={(value:string)=>{setAnswer(value);rememberQuestionDraft(ws.id,q.id,value)}}/>
        ...
        {!feedback||feedback.retry_allowed?<><div className="question-navigation"><button type="button" disabled={!previous} onClick={()=>safe(previousQuestion)}><ChevronLeft size={19}/> Previous question</button>{canFinishWithSkipped&&<button type="button" className="finish-skipped" onClick={()=>safe(finish)}>Finish worksheet with skipped questions</button>}</div><div className="support-actions"><button className="hint-button" disabled={hintBusy} onClick={()=>safe(requestHint)}><Lightbulb size={19}/>{hintBusy?'Getting help…':q.hint_count>=3?'Review final hint':q.hint_count===2?'Show worked next step':q.hint_count===1?'Show a strategy':'Give me a hint'}</button><small>Hints become gradually stronger and do not reduce the score.</small></div><div className="question-actions"><button className="skip" onClick={()=>safe(skip)}><SkipForward size={19}/> Skip for now</button><button type="button" className="primary" disabled={!String(answer ?? '').trim()} onClick={()=>safe(submit)}>Check answer</button></div>{feedback&&<div className="feedback optional-tutor-feedback wrong"><h3>↻ You can try another answer now</h3><p>{feedback.message}</p><button type="button" onClick={()=>{setFeedback(null);setAnswer('')}}>Dismiss this prompt</button></div>}</>:<div className={'feedback '+(feedback.correct?'correct':'wrong')}><h3>{feedback.correct?'✅ Great job!':'❌ '+feedback.message}</h3>{feedback.working&&<p>{feedback.working}</p>}{!feedback.retry_allowed&&ws.test_mode&&<TestQuestionFeedback worksheetId={ws.id} question={q}/>} {!feedback.retry_allowed&&!ws.test_mode&&<ConfidenceCheck questionId={q.id}/>}<button className="primary" onClick={()=>safe(next)}>{ws.counts.remaining<=1?'Finish worksheet':'Next question'} <ChevronRight size={18}/></button></div>}
      </section>
...
function Parent({user,logout}:{user:User;logout:()=>void}){
  ...
  const[insight,setInsight]=useState<any>(null),[recommendations,setRecommendations]=useState<any>(null);
  ...
  const load=()=>{setError('');Promise.all([req('/dashboard/parent'),req('/settings'),req('/backups'),req('/learning/parent-insight-v0240').catch(()=>null),req('/learning/recommendations').catch(()=>null)]).then(([dashboard,nextSettings,nextBackups,nextInsight,nextRecommendations])=>{setD(dashboard);setSettings(nextSettings);setBackups(nextBackups);setInsight(nextInsight);setRecommendations(nextRecommendations)}).catch((e:Error)=>setError(e.message))};
  useEffect(load,[]);
  ...
  {recommendations&&<section className="panel mentor-recommendations"><div className="panel-heading"><div><p className="eyebrow">LEARNING INTELLIGENCE</p><h2>What to practise next</h2><p>These early recommendations use prerequisite links, misconception signals and supported evidence. Parent tests are excluded.</p></div><span className="status-pill developing">{recommendations.evidence_events} evidence events</span></div><div className="concern-grid">{recommendations.recommendations.map((item:any)=><div key={`${item.type}-${item.title}`}><b>{item.title}</b><span>{item.reason}</span><strong>{item.minutes} min</strong><small>{item.type==='misconception'?'Recurring misconception':'Spaced retrieval'}</small></div>)}</div></section>}

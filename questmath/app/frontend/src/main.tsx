...
import {ErrorNotice, StudentDestination, StudentMobileNavigation, StudentSection, WorksheetHistory} from './student-foundation';
...
import {setupKeyboardViewport} from './keyboard-viewport';
...
const LEARNER_SKILL_LABELS:Record<string,string>={fact_recall_addition:'Addition facts',fact_recall_subtraction:'Subtraction facts',efficient_add_subtract:'Addition and subtraction strategies',efficient_multiply_divide:'Multiplication and division strategies',equivalent_fractions:'Equivalent fractions',fraction_story_compare:'Comparing fractions',decimal_place_value:'Decimal place value',number_sequences:'Number patterns',error_analysis_regrouping:'Checking a calculation',perimeter:'Perimeter',area:'Area',probability:'Chance and probability',statistics:'Reading data',unknown:'Unknown values'};
function learnerSkillLabel(skill:string,topic:string){const key=String(skill||'').split(':').pop()||'';if(LEARNER_SKILL_LABELS[key])return LEARNER_SKILL_LABELS[key];const fallback=key.replaceAll('_',' ').trim();if(fallback)return fallback.replace(/\b\w/g,letter=>letter.toUpperCase());return topic==='algebra'?'Number patterns':topic==='measurement'?'Measurement':topic==='probability'?'Chance and probability':topic==='statistics'?'Reading data':'Mathematics';}
function learnerQuestionSummary(q:Question){const summary=String(q.summary||'').trim();return summary&&!/^VC2M/i.test(summary)&&!summary.includes(':')&&!/^[a-z0-9_]+$/i.test(summary)?summary:learnerSkillLabel(q.skill,q.topic);}
...
function statusLabel(status:QuestionStatus){return({not_started:'Ready',current:'Current',skipped:'Skipped',correct:'Correct',incorrect:'Needs another try',retry_available:'Try again'} as any)[status]}
function QuestionOverview({ws,activeId,close,goTo}:{ws:WorksheetData;activeId:number;close:()=>void;goTo:(id:number)=>void}){return <div className="modal-backdrop"><section className="overview-modal"><div className="modal-title"><div><p className="eyebrow">WORKSHEET MAP</p><h2>All questions</h2></div><button className="icon-button" aria-label="Close all questions" onClick={close}><X/></button></div><div className="question-table"><div className="question-table-head"><b>#</b><b>Question</b><b>Status</b></div>{ws.questions.map(q=>{const locked=['correct','incorrect'].includes(q.status);return <button key={q.id} className={'question-row '+(q.id===activeId?'active':'')} aria-current={q.id===activeId?'true':undefined} disabled={locked} onClick={()=>goTo(q.id)}><span>{q.position+1}</span><span><b>{learnerQuestionSummary(q)}</b><small>{learnerSkillLabel(q.skill,q.topic)}{q.hint_count?' · 💡 '+q.hint_count:''}</small></span><span className={'status-pill '+q.status}>{statusLabel(q.status)}</span></button>})}</div><p className="overview-note">Completed questions are read-only. Skipped and unanswered questions can be opened.</p></section></div>}

...
export function TargetedLearningPreview(){
  ...
  const load=()=>{setError('');apiRequest<Plan>('/learning/session-plan-v0460').then(setData).catch((e:Error)=>setError(e.message))};
  useEffect(load,[]);
...
export function TargetedCompletion({worksheetId,back}:{worksheetId:number;back:()=>void}){
  ...
  useEffect(()=>{apiRequest<Summary>(`/worksheets/${worksheetId}/targeted-summary-v0460`).then(setData).catch(()=>setData({available:false}))},[worksheetId]);

...
function visualMarkup(v:any){
  ...
  if(v.type==='clock'){
    ...
    return `<div class="mq-visual clock" role="img" aria-label="Analogue clock"><div class="clock-face"><i class="hour" style="transform:rotate(${hh}deg)"></i><i class="minute" style="transform:rotate(${mh}deg)"></i><b>12</b><b>3</b><b>6</b><b>9</b></div></div>`;
  }
  if(v.type==='angle')return `<div class="mq-visual angle"><svg viewBox="0 0 240 150" role="img" aria-label="Angle diagram"><line x1="60" y1="120" x2="195" y2="120"/><line x1="60" y1="120" x2="${60+110*Math.cos(v.degrees*Math.PI/180)}" y2="${120-110*Math.sin(v.degrees*Math.PI/180)}"/><path d="M90 120 A30 30 0 0 0 ${60+30*Math.cos(v.degrees*Math.PI/180)} ${120-30*Math.sin(v.degrees*Math.PI/180)}"/></svg></div>`;
  if(v.type==='bar_chart'){
    const m=Math.max(...v.values);
    return `<div class="mq-visual bar-chart" role="img" aria-label="Bar chart">${v.labels.map((x:string,i:number)=>`<div><span style="height:${Math.max(12,v.values[i]/m*120)}px"></span><b>${v.values[i]}</b><small>${x}</small></div>`).join('')}</div>`;
  }
...
async function ensureRequiredVisual(){
  ...
  try{
    ...
    if(inherentlyVisual){
      ...
      warning.innerHTML='<strong>Visual unavailable</strong><p>This question needs a diagram that is missing. Please skip this question for now so it can be reviewed.</p>';

...
export function StudentProgress({onStart}:{onStart?:()=>void}){
  ...
  const visibleRows=useMemo(()=>{
    const rows=data?.learning_now||[];
    if(showAllAreas)return rows;
    const meaningful=rows.filter(row=>row.state.key!=='not_enough_evidence');
    return meaningful.length ? meaningful.slice(0,6) : rows.slice(0,3);
  },[data,showAllAreas]);
  ...
  return <section id="mq-student-progress" className="panel student-progress" aria-labelledby="student-progress-title">
    ...
    {data&&<>
      ...
      {data.learning_now.length>visibleRows.length&&<button type="button" className="student-progress-detail-toggle" aria-expanded={showAllAreas} onClick={()=>setShowAllAreas(!showAllAreas)}>{showAllAreas?'Show priority learning only':'View all learning areas'}</button>}

...
const payload = {
  ...
  learning_now: [
    ...
    {code:'B',strand:'Algebra',title:'Patterns',questions:9,last_practised:null,review_due:false,state:{key:'getting_stronger',label:'Getting stronger',message:'You are solving these independently more often.',target_skill:'number_sequences',evidence:{questions:9,independent_accuracy:78,eventual_accuracy:89,support_dependency:11}}},
...
describe('StudentProgress', () => {
  beforeEach(() => {apiRequest.mockReset(); apiRequest.mockResolvedValue(payload)});

  it('renders learner states and the actual recommendation explanation without parent jargon', async () => {
    render(<StudentProgress onStart={vi.fn()}/>);
  ...
  it('starts the existing best-next-step action', async () => {
    const onStart=vi.fn();
    render(<StudentProgress onStart={onStart}/>);
    await screen.findByText('Review Fractions');
    fireEvent.click(screen.getByRole('button',{name:/start best next step/i}));
    expect(onStart).toHaveBeenCalledTimes(1);
  });

  it('uses one progress API request instead of duplicating calculations in React', async () => {
    render(<StudentProgress onStart={vi.fn()}/>);

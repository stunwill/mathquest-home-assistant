...
async function restartSkipped(id:number){const worksheet=await api(`/worksheets/${id}/restart-skipped`,{method:'POST'});setActive(Number(worksheet.id));sessionStorage.setItem('mq_open_worksheet','1');location.reload()}
...
async function enhanceHistory(){
  ...
  section.querySelectorAll('[data-restart]').forEach((b:any)=>b.addEventListener('click',()=>restartSkipped(Number(b.dataset.restart))));
}
...
async function renderCalendar(force=false){
  const panels=[...document.querySelectorAll('.panel')];const target=(document.querySelector('.mq-v0160-calendar')||panels.find(p=>p.querySelector('h2')?.textContent?.trim()==='Completion calendar')) as HTMLElement|undefined;if(!target)return;
  const start=localDate(rangeStart);if(calendarLoading||(!force&&target.dataset.calendarStart===start))return;calendarLoading=true;
  try{
  target.classList.add('mq-v0160-calendar');
  const data=await api(`/learning/week-v0160?start=${start}`);const currentMonday=monday();const canForward=rangeStart<currentMonday;target.dataset.calendarStart=start;
  target.innerHTML=`<div class="mq-cal-head"><button data-shift="-7">« 1 week</button><button data-shift="-1">‹ 1 day</button><h2>${dateLabel(data.start)} – ${dateLabel(data.end)}</h2><button data-shift="1" ${canForward?'':'disabled'}>1 day ›</button><button data-shift="7" ${canForward?'':'disabled'}>1 week »</button></div><div class="mq-cal-days">${data.days.map((d:any)=>{const any=d.worksheets.length>0,complete=any&&d.worksheets.every((w:any)=>!!w.completed_at),progress=any&&!complete;return `<article class="mq-cal-day${d.is_today?' today':''}${complete?' complete':''}${progress?' in-progress':''}${d.is_future?' future':''}"><h3>${dateLabel(d.date)}</h3><div class="mq-cal-stats">${d.questions?`<span>${d.questions} questions · ${d.accuracy??0}%</span><span>${d.correct} correct · ${d.incorrect} incorrect</span><span>💡 ${d.hints} · ⭐ ${d.xp} · ${mins(d.elapsed_seconds)}</span>`:'<span>No learning activity</span>'}</div><div class="mq-cal-ws">${d.worksheets.map((w:any)=>`<button data-${w.completed_at?'view':'continue'}="${w.id}">${w.display_title} · ${w.answered}/${w.total} ${w.completed_at?'✓':'→'}</button>${w.restartable_skipped?`<button data-restart="${w.id}">Restart ${w.skipped} skipped</button>`:''}`).join('')}</div></article>`}).join('')}</div>`;
  target.querySelectorAll('[data-shift]').forEach((b:any)=>b.addEventListener('click',()=>{const n=Number(b.dataset.shift);let next=addDays(rangeStart,n);if(next>currentMonday)next=currentMonday;rangeStart=next;void renderCalendar(true)}));
  target.querySelectorAll('[data-continue]').forEach((b:any)=>b.addEventListener('click',()=>continueWorksheet(Number(b.dataset.continue))));target.querySelectorAll('[data-view]').forEach((b:any)=>b.addEventListener('click',()=>viewWorksheet(Number(b.dataset.view))));target.querySelectorAll('[data-restart]').forEach((b:any)=>b.addEventListener('click',()=>restartSkipped(Number(b.dataset.restart))));

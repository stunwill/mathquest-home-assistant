...
def test_release_notes_extract_current_version_section_only():
    ...
    assert notes.startswith('- Added parent reporting')
    assert 'outcome growth' in notes
    assert 'Home Assistant service token' in notes
    assert 'graceful unavailable responses' in notes
...
def test_required_version_locations_agree():
    ...
    assert set(versions.values()) == {'0.24.0'}

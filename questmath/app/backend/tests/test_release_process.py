...
def test_release_notes_extract_current_version_section_only():
    ...
    assert notes.startswith('- Added parent-only test worksheet creation')
    assert 'isolated from her progress' in notes
    assert 'semantic release traceability' in notes
    assert 'diagnostic growth reporting' in notes
...
def test_required_version_locations_agree():
    ...
    assert set(versions.values()) == {'0.25.0'}

...
def test_release_notes_extract_current_version_section_only():
    ...
    assert notes.startswith('- Added outcome-level mastery')
    assert 'spaced-review due dates' in notes
    assert 'prerequisite routing' in notes
    assert 'recommended sessions' in notes
...
def test_required_version_locations_agree():
    ...
    assert set(versions.values()) == {'0.23.0'}

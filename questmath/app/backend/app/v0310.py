...
from . import v0120, v0200, v0230, v0260, v0280, v0290, v0301
...
def _arithmetic_tutoring(question: legacy.Question) -> dict[str, Any] | None:
    ...
    if operation == 'addition':
        a_h, a_t, a_o = a // 100 * 100, (a % 100) // 10 * 10, a % 10
        b_h, b_t, b_o = b // 100 * 100, (b % 100) // 10 * 10, b % 10
        regroup = a_o + b_o >= 10
        bridge = a_o + b_o == 10
        ...
        hint2 = (
            ...
            f'{a} = ' + ' + '.join(str(x) for x in (a_h, a_t, a_o) if x) + '\n'
            f'{b} = ' + ' + '.join(str(x) for x in (b_h, b_t, b_o) if x) + '\n'
            f'Start by combining the ones.'
        )
        if regroup:
            hint3 = f'{a_o} + {b_o} makes at least one whole ten. Regroup that ten, then combine the remaining tens and hundreds.'
        else:
            hint3 = f'Combine {a_o} + {b_o} first, then combine the tens and hundreds. Keep each place value lined up.'
        teach_steps = [
            {'label': 'Notice', 'text': f'This is addition. A useful strategy here is to {strategy}.'},
            {'label': 'Partition', 'math': [
                f'{a} = ' + ' + '.join(str(x) for x in (a_h, a_t, a_o) if x),
                f'{b} = ' + ' + '.join(str(x) for x in (b_h, b_t, b_o) if x),
            ]},
            {'label': 'Your turn', 'text': f'What do {a_o} and {b_o} make together?'},
        ]
        return {'strategy': strategy, 'hints': [hint1, hint2, hint3], 'teach_steps': teach_steps}
...
def tutoring_content(question: legacy.Question) -> dict[str, Any]:
    ...
    return {
        ...
        'hints': [
            plan['stages'][0],
            plan['stages'][1],
            v0200.hint_text_v0200(question, 3),
        ],
        'teach_steps': [
...
def create_worksheet_v0310(session: Session, student_id: int, selected: str, **kwargs: Any) -> legacy.Worksheet:
    ...
    for question in worksheet.questions:
        payload = _payload(question)
        payload.setdefault('mentor_context', _question_context(question))
        question.payload = json.dumps(payload)

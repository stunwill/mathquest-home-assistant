...
- Parent Learning Insight now tolerates legacy or partial payloads that omit curriculum outcome evidence.
- Missing outcome evidence displays a clear empty state instead of crashing the Parent Dashboard.
- The fix includes regression coverage and does not change student worksheets, adaptive learning or stored learner evidence.
...
- Student worksheet-map entries use learner-safe mathematical descriptions instead of curriculum codes or internal skill identifiers.
- Current, Ready, Correct, Try again and Skipped states are clearer, and the All questions dialog has an accessible close label.
- Generic praise is omitted when the learning engine has no specific insight, while mathematical explanations remain.
- Physical iPhone/iPad acceptance remains pending until tested on real devices.

## v0.47.2 mathematics-first worksheet UX

- The active question and answer interaction now appear before secondary worksheet tools.
- Internal topic/difficulty text is no longer exposed on the student worksheet.
- Skip for now has secondary visual weight, diagnostic selection is concise, and iPad keyboard-aware layout remains available.
- Physical iPhone/iPad acceptance remains pending until tested on real devices.

## v0.47.1 iPad keyboard answer visibility

- The worksheet keeps the active answer field and Check answer action together when the iPadOS software keyboard reduces the landscape viewport.
- Tablet landscape uses a horizontal answer interaction group; phones retain stacked controls.
- Physical iPhone/iPad acceptance remains pending until tested on real devices.

## v0.47.0 follow-through\n\n- Targeted worksheets retain their learning purpose and pre-session evidence, allowing completion and parent views to explain what changed after real practice.\n- Student completion remains learner-safe, while Parent Dashboard can inspect support use, independent checks and before/after evidence.\n\n## Included
